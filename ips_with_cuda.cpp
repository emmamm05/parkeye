...
IPS_with_cuda::IPS_with_cuda()
{
}


Mat IPS_with_cuda::applyBlur(Mat src){
    Mat dst = src.clone();

    /* cv::Mat --> cv::gpu::GpuMat */
    gpu::GpuMat GpuMat_src(src);
    gpu::GpuMat GpuMat_dst;

    clock_t startStep = clock();
    gpu::GaussianBlur( GpuMat_src, GpuMat_dst, Size( Constants::BLUR_KERNEL_LENGTH, Constants::BLUR_KERNEL_LENGTH ), 0, 0 );
    ...
    /* cv::Mat <-- cv::gpu::GpuMat */
    GpuMat_dst.download(dst);
    return dst;
}

Mat IPS_with_cuda::applyLaplacian(Mat src){
    Mat dst = src.clone();
...
Mat IPS_with_cuda::applyEdge(Mat src){
    Mat dst = src.clone();
    Mat src_gray = src.clone();
    cvtColor( src, src_gray, COLOR_BGR2GRAY );

    /* cv::Mat --> cv::gpu::GpuMat */
    gpu::GpuMat GpuMat_src(src_gray);
    gpu::GpuMat GpuMat_dst;

    clock_t startStep = clock();
    gpu::Canny( GpuMat_src, GpuMat_dst, Constants::EDGES_LOW_THRESHOLD, Constants::EDGES_LOW_THRESHOLD*Constants::EDGES_RATIO, Constants::EDGES_KERNEL_SIZE );
    clock_t stopStep = clock();
    double elapsedStep = (double)difftime(startStep, stopStep) * 1000.0 / CLOCKS_PER_SEC;
...
Mat IPS_with_cuda::applySubs(Mat raw_src, Mat ref_src){
    Mat dst = raw_src.clone();
    gpu::GpuMat GpuMat_dst;

    /* cv::Mat --> cv::gpu::GpuMat */
    gpu::GpuMat GpuMat_raw_src(raw_src);
    gpu::GpuMat GpuMat_raw_dst;
    gpu::GpuMat GpuMat_ref_src(ref_src);
    gpu::GpuMat GpuMat_ref_dst;

    clock_t startStep = clock();
    gpu::absdiff(GpuMat_raw_src,GpuMat_ref_src,GpuMat_dst);
    clock_t stopStep = clock();
    double elapsedStep = (double)difftime(startStep, stopStep) * 1000.0 / CLOCKS_PER_SEC;
...
Mat IPS_with_cuda::applyContourns(Mat src){
...
int IPS_with_cuda::processLaplacian(){
    //Raw Image
...
int IPS_with_cuda::processContourns(){
